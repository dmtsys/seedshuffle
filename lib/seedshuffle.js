...
function getChunks(password, { sha256 }) {
...
function shuffle(mnemonic, password, { sha256 }) {
  if (!password || password.trim() == '') {
    return mnemonic;
  }

  const chunks = getChunks(password, { sha256 });
...
function deshuffle(shuffled, password, { sha256 }) {
  if (!password || password.trim() == '') {
    return shuffled;
  }
  ...
  const chunks = getChunks(password, { sha256 }).slice(0, wordCount).reverse();
